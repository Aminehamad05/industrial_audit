...
export const SupervisorCreateAuditPanel: React.FC<{ onCreated: () => void }> = ({ onCreated }) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.supervisor.getAuditors().then((r) => r.json()).then((d) => setAuditors(d.auditors || [])),
  ...
  const handleCreateAudit = async () => {
    ...
    if (!auditType || !auditTarget || !selectedPlantId || !selectedShift || !startDate) {
    ...
    try {
      ...
      if (selectedAuditorId) {
        body.auditorId = selectedAuditorId;
      }

      const res = await api.audits.create(body);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="px-3 py-2 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-hutchinson-blue/20 focus:border-hutchinson-blue"
              >
                <option value="">{t('select')} {t('auditor')}</option>
                {auditors.map((a) => (
                  <option key={a.id} value={a.id}>
                    {a.fullName || a.username} {a.plantName ? `— ${a.plantName}` : ''}
                  </option>
                ))}
              </select>
            </div>
...
export default SupervisorCreateAuditPanel;