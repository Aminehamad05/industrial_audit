...
adminRouter.get("/users", async (req, res) => {
  ...
  if (statusFilter && !ACCOUNT_STATUSES.includes(statusFilter as AccountStatus)) {
    return res.status(400).json({ error: `Invalid status. Must be one of: ${ACCOUNT_STATUSES.join(", ")}` });
  }
  const users = await userRepo.find({
  where: statusFilter
    ? { accountStatus: statusFilter as AccountStatus }
    : {},
  select: {
    id: true,
    username: true,
...
async function transitionStatus(
  ...
  if (!allowedFromStatuses.includes(user.accountStatus)) {
    throw new AppError(
      409,
      `Cannot move user from '${user.accountStatus}' to '${toStatus}'`
    );
  }
