...
async function seed() {
    ...
    try {
        ...
        if (plantCount === 0) {
            plant1 = await prisma_1.prisma.plant.create({
                data: { designationPlant: "FMS-Alpha" },
            });
            plant2 = await prisma_1.prisma.plant.create({
                data: { designationPlant: "FMS-Beta" },
            });
            plant3 = await prisma_1.prisma.plant.create({
                data: { designationPlant: "A&D-Gamma" },
            });
            console.log(`✅ Created plants: ${plant1.designationPlant}, ${plant2.designationPlant}, ${plant3.designationPlant}\n`);
        }
        else {
            // Get existing plants
        ...
        // Hash passwords
        const adminPasswordHash = await bcryptjs_1.default.hash("admin123", BCRYPT_ROUNDS);
        const auditor1PasswordHash = await bcryptjs_1.default.hash("auditor123", BCRYPT_ROUNDS);
        const auditor2PasswordHash = await bcryptjs_1.default.hash("auditor123", BCRYPT_ROUNDS);
        const supervisorPasswordHash = await bcryptjs_1.default.hash("supervisor123", BCRYPT_ROUNDS);
        const admin = await prisma_1.prisma.aspnet_Users.upsert({
        ...
        const auditor1 = await prisma_1.prisma.aspnet_Users.upsert({
        ...
        const auditor2 = await prisma_1.prisma.aspnet_Users.upsert({
        ...
        const supervisor = await prisma_1.prisma.aspnet_Users.upsert({
        ...
        console.log(`✅ Created/Updated users: ${admin.UserName}, ${auditor1.UserName}, ${auditor2.UserName}, ${supervisor.UserName}\n`);
        // 3. Create Audits
        ...
        if (auditCount === 0) {
            ...
            console.log(`✅ Created audits: ${audit1.auditTypeName}, ${audit2.auditTypeName}, ${audit3.auditTypeName}\n`);
        }
        else {
            const existingAudits = await prisma_1.prisma.audits.findMany({ take: 3 });
            audits.push(...existingAudits);
            console.log(`✅ Using existing audits\n`);
        }
        // 4. Create Audit Details
        console.log("📝 Creating audit details...");
        const detailCount = await prisma_1.prisma.audit_details.count();
        const details = [];
        if (detailCount === 0 && audits.length > 0) {
            // Details for Audit 1
            for (let i = 1; i <= 5; i++) {
                const detail = await prisma_1.prisma.audit_details.create({
                    data: {
                        auditId: audits[0].id,
                        groupPosition: 1,
            ...
            const existingDetails = await prisma_1.prisma.audit_details.findMany({ take: 10 });
        ...
        if (statusCount === 0) {
            const statusOpen = await prisma_1.prisma.setts_action_status.create({
                data: { id_status: 1, etat_status: "Open" },
            });
            const statusInProgress = await prisma_1.prisma.setts_action_status.create({
                data: { id_status: 2, etat_status: "In Progress" },
            });
            const statusClosed = await prisma_1.prisma.setts_action_status.create({
                data: { id_status: 3, etat_status: "Closed" },
            });
            console.log(`✅ Created action statuses: ${statusOpen.etat_status}, ${statusInProgress.etat_status}, ${statusClosed.etat_status}\n`);
        }
        else {
            console.log(`✅ Using existing action statuses\n`);
        }
        // Get action statuses for use in actions
        const statuses = await prisma_1.prisma.setts_action_status.findMany({ take: 3 });
        // 6. Create Actions
        ...
        if (actionCount === 0 && details.length > 0 && statuses.length > 0) {
            for (let i = 0; i < 3 && i < details.length; i++) {
                const action = await prisma_1.prisma.actions.create({
                    data: {
            ...
            const existingActions = await prisma_1.prisma.actions.findMany({ take: 3 });
        ...
        if (scheduleCount === 0) {
            ...
            console.log(`✅ Created schedules: ${schedule1.scheduleName}, ${schedule2.scheduleName}, ${schedule3.scheduleName}\n`);
        }
        else {
        ...
        if (typeCount === 0) {
            const auditType1 = await prisma_1.prisma.setts_audit_types.create({
                data: { audit_type: "SAFETY", name: "Safety Audit" },
            });
            const auditType2 = await prisma_1.prisma.setts_audit_types.create({
                data: { audit_type: "MAINTENANCE", name: "Maintenance Audit" },
            });
            const auditType3 = await prisma_1.prisma.setts_audit_types.create({
                data: { audit_type: "COMPLIANCE", name: "Compliance Audit" },
            });
            console.log(`✅ Created audit types: ${auditType1.name}, ${auditType2.name}, ${auditType3.name}\n`);
        }
        else {
        ...
        if (groupCount === 0) {
            const group1 = await prisma_1.prisma.setts_audit_question_groups.create({
                data: {
                    audit_type: "SAFETY",
                    group_position: 1,
            ...
            const group2 = await prisma_1.prisma.setts_audit_question_groups.create({
                data: {
            ...
            console.log(`✅ Created audit question groups: ${group1.group_name}, ${group2.group_name}\n`);
        }
        else {
